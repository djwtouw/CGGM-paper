...
settings <- data.frame(
  n = c(240, 480, 240, 240, 240),  # number of observations
  p = c( 30,  60,  30,  30,  30),  # number of variables
  B = c(  3,   3,   5,   6,  10),  # number of blocks
  R = c(100,  50, 100, 100, 100)   # number of simulation runs
)
...
results_null <- mapply(function(n, p, B, R) {
  ...
  results_list <- lapply(1:R, function(r) {
    ...
    df_CGGM <- tryCatch({
      ...
      data.frame(Run = r, n = n, p = p, B = B, Method = "CGGM-refit",
  ...
  ## combine results for current setting into data frame
  results <- do.call(rbind, results_list)

  ## store results for current setting
...
}, n = settings$n, p = settings$p, B = settings$B, R = settings$R,
SIMPLIFY = FALSE, USE.NAMES = FALSE)
