...
generate_offdiagonal_block <- function(nrow, ncol, pi, theta) {
  matrix(ifelse(runif(nrow * ncol) < pi, theta, 0), nrow = nrow, ncol = ncol)
}
    ...
    for (j in seq_len(B)) {
      for (i in seq_len(B)) {
        if (i == j) Theta[[j]][[i]] <- generate_diagonal_block(p_b[i], theta_w)
          ...
          Theta[[j]][[i]] <- generate_offdiagonal_block(p_b[i], p_b[j],
                                                        pi, theta_b)
...
## store results
file <- "simulations/results/results_blockdiagonal_unbalanced_n=%d_p=%d_B=%d_R=%d.RData"
save(results, n, p_b, p, B, theta_w, theta_b, session_info,
