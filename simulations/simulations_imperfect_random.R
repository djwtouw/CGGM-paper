...
theta_w <- c(0.4, 0.6)  # (off-diagonal) elements within blocks
theta_b <- c(0.2, 0.3)  # non-zero elements between blocks
...
edges <- combn(B, 2)                      # edges in full aggregated graph
...
# function to generate blocks of zeros for precision matrix
...
results_list <- lapply(1:R, function(r) {
  ...
  # select one non-zero edge in the aggregated graph at random
  connected <- edges[, sample.int(ncol(edges), 1)]
...
## store results
file <- "simulations/results/results_imperfect_random_n=%d_p=%d_B=%d_R=%d.RData"
