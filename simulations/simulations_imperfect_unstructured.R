...
theta <- c(0.2, 0.3)  # non-zero elements of precision matrix
R <- 100              # number of simulation runs

## additional parameters
B <- p         # number of blocks
blocks <- 1:B  # true block structure
...
results_list <- lapply(1:R, function(r) {
  ...
  # set nonzero offdiagonal elements with a certain probability
  prob_connected <- runif(choose(p, 2))
  connected <- which(prob_connected < pi)
  Theta[which(lower)[connected]] <- runif(length(connected),
                                          min = theta[1],
                                          max = theta[2])
...
## store results
file <- "simulations/results/results_imperfect_unstructured_n=%d_p=%d_B=%d_R=%d.RData"
save(results, n, p, B, pi, theta, session_info, file = sprintf(file, n, p, B, R))
