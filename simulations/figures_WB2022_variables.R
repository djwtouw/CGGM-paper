...
## parameter settings
n <- c(120, 240, 480)  # number of observations
p <- c( 15,  30,  60)  # number of variables
...
df_results <-
  ...
  }, n = n, p = p, SIMPLIFY = FALSE, USE.NAMES = FALSE) %>%
  bind_rows() %>%
  ...
  mutate(Design = factor(Design, levels = design),
         p = factor(sprintf(p_label, p), levels = p_levels),
...
df_true <- rbind(data.frame(Design = design, Criterion = "B_hat", Value = B),
                 data.frame(Design = design, Criterion = "ARI", Value = 1)) %>%
  mutate(Design = factor(Design, levels = design),
         Criterion = factor(criterion_labels[Criterion],
                            levels = criterion_labels))
...
plt <- ggplot() +
  ...
  facet_grid(Criterion ~ p, scales = "free_y") +
  scale_x_discrete(labels = label_custom()) +
  facetted_pos_scales(
    y = list(NULL,
             scale_y_continuous(breaks = seq(from = 1, to = p[1], by = 4),
...
## save plot to file
pdf(file = "simulations/figures/WB2022_variables.pdf",
