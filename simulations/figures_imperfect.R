...
df_true <- rbind(data.frame(Design = designs, Criterion = "B_hat", Value = B),
                 data.frame(Design = designs, Criterion = "ARI", Value = 1)) %>%
  mutate(Design = factor(design_labels[Design],
                         levels = design_labels),
         Criterion = factor(criterion_labels[Criterion],
                            levels = criterion_labels))
...
plt <- ggplot() +
  ...
  facet_grid(Criterion ~ Design, scales = "free_y") +
...
## save plot to file
pdf(file = "simulations/figures/imperfect.pdf",
