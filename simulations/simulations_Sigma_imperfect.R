...
sigma_w <- c(0.4, 0.6)  # (off-diagonal) elements within blocks
sigma_b <- c(0.2, 0.3)  # non-zero elements between blocks
  ...
  for (j in seq_len(B)) {
    for (i in seq_len(B)) {
      if (i == j) Sigma[[j]][[i]] <- generate_diagonal_block(p_b[i], sigma_w)
        ...
        Sigma[[j]][[i]] <- generate_zero_block(p_b[i], p_b[j])
...
## store results
file <- "simulations/results/results_Sigma_imperfect_n=%d_p=%d_B=%d_R=%d.RData"
