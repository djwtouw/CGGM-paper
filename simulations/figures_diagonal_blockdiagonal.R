...
designs_main <- c("diagonal", "blockdiagonal")
designs_variation <- c("balanced", "unbalanced")
designs <- expand.grid(
  Main = designs_main,
  Variation = designs_variation
)
...
file <- paste(path, "results_%s_%s_n=%d_p=%d_B=%d_R=%d.Rdata", sep = "/")

## nice labels for methods and evaluation criteria
main_labels <- c(diagonal = "Clustering structure on diagonal",
                 blockdiagonal = "Blockdiagonal structure")
variation_labels <- c(balanced = "Balanced",
                      unbalanced = "Unbalanced")
method_labels <- c("CGGM-raw" = "CGGM-raw",
                   "CGGM-refit" = "CGGM-refit",
...
colors <- hue_pal()(6)[c(4, 4, 6, 6, 6, 1, 5)]


##  load results
df_results <-
  mapply(function(main, variation) {
    load(sprintf(file, main, variation, n, p, B, R))
    data.frame(Main = main, Variation = variation, results)
  }, main = designs$Main, variation = designs$Variation,
  SIMPLIFY = FALSE, USE.NAMES = FALSE) %>%
  bind_rows() %>%
  select(Main:Method, Error, B_hat, ARI) %>%
  pivot_longer(cols = Error:ARI, names_to = "Criterion",
               values_to = "Value") %>%
  mutate(Main = factor(main_labels[Main],
                       levels = main_labels),
         Variation = factor(variation_labels[Variation],
                            levels = variation_labels),
...
df_estimation_means <- df_estimation %>%
  group_by(Main, Variation, n, p, B, Method, Criterion) %>%
...
df_aggregation_frequencies <- df_aggregation %>%
  group_by(Main, Variation, n, p, B, Method, Criterion, Value) %>%
  summarize(Frequency = n(),
            .groups = "drop") %>%
  left_join(df_aggregation_means,
            by = c("Main", "Variation", "n", "p", "B", "Method", "Criterion")) %>%
  mutate(Frequency = Frequency / Nr_Runs)

# construct data frame of true values for reference lines
df_true <- rbind(data.frame(designs, Criterion = "B_hat", Value = B),
                 data.frame(designs, Criterion = "ARI", Value = 1)) %>%
  mutate(Main = factor(main_labels[Main],
                       levels = main_labels),
         Variation = factor(variation_labels[Variation],
                            levels = variation_labels),
...
plt <- ggplot() +
  ...
  facet_nested(Criterion ~ Main + Variation, scales = "free_y") +
...
## save plot to file
pdf(file = "simulations/figures/diagonal_blockdiagonal.pdf",
    width = 6.45, height = 4.5)
print(plt)
dev.off()
